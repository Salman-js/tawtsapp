...
import { getUsersBySearch } from '../api/user';
...
const UsersSearchTab = () => {
  ...
  const searchedUsersQuery = useQuery({
    queryKey: ['users', 'search', searchString],
    queryFn: () =>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            colors={['white']}
            refreshing={searchedUsersQuery.isInitialLoading}
            onRefresh={() => {
              searchedUsersQuery.refetch();
            }}
          />
        }
      >
        {searchedUsersQuery.data?.length ? (
          searchedUsersQuery.data.map((userItem) => (
            <SearchUserItem key={userItem.id} item={userItem} />
          ))
        ) : (
